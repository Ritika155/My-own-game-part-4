class Game {
  ...
  play(){
   // console.log("in play funcion....")
    ...
    if(allPlayers !== undefined){
      ...
      if (! beachWavesSound.isPlaying()) {
        beachWavesSound.play();
      }
  ...
  spawnCoins(){
    console.log("spwaning coins");
 
    var x = Math.round(random(width/2+150,width/2-150));
    var y = Math.round(random(-height*3,4.5,height-100));

      for (let index = 0; index < 20; index++) {
    
        var coinSprite = createSprite(x,y)
        coinSprite.addImage("coin",coinImg)
